export class PauseScene extends Phaser.Scene {
    ...
    create() {
        ...
        if (this.parent == "ARENA") {
            this.add.rectangle(0, 0, 950, 500, 0x000000, 0.2).setOrigin(0).setDepth(-1);
        }

        const y = this.parent == "ARENA" ? 225 : 360
        const s = this.parent == "ARENA" ? 1.1 : 0.7
        const s2 = this.parent == "ARENA" ? 0 : 115
        let reset = this.add.image(250 + s2, y, 'reset').setInteractive().setScale(s);
        ...
        let home = this.add.image(685 - s2, y, 'home2').setInteractive().setScale(s).setAngle(-1.9);
        ...
        let play = this.add.image(480, y, 'play').setInteractive().setScale(s + 0.2);
